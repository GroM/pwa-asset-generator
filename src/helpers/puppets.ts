...
): Promise<LaunchScreenSpec[]> =>
{
	...
	const splashScreenData = await page.evaluate((dimensionRegexDef) =>
	{
		const dimensionRegex = new RegExp(dimensionRegexDef, 'm');
		...
		): { width: number; height: number; scaleFactor: number } =>
		{
			...
			if (!regexMatch?.length)
			{
				...
				// throw Error('Regex match failed while scraping the specs `' + dimensionRegex.source + '` on `' + val + '`');
			}
	...
	}, constants.APPLE_HIG_SPLASH_SCREEN_FALLBACK_DATA_REGEX);
...
): Promise<SavedImage[]> =>
{
	...
	if( !((await file.exists(output)) && (await file.isPathAccessible(output, file.WRITE_ACCESS))) )
	{
